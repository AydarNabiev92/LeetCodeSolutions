...
public class SolutionThree {
    ...
    public static int[] twoSum(int[] nums, int target) {
        ...
        for (int i = 0; i <= nums.length - 1; i++) {
            ...
            // Check subtraction result against others, don't sum it with itself
            for (int j = i + 1; j <= nums.length - 1; j++) {
                if (candidateSoulmate == nums[j]) {
                    return new int[]{i, j};
                }
            }
        }

        // This isn't the solution, but the task seems to always give nums with required
        // elements for target
        return new int[]{0, 0};
    }
}
