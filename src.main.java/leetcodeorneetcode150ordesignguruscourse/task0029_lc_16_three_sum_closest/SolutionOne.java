...
public class SolutionOne {
        ...
        for (int i = 0; i <= nums.length - 3; i++) {
            ...
            while (left < right) {
                ...
                if (actualSum == requiredForSum) {
                    ...
                    // update closest correctly, it must be closest to the target
                    if (Math.abs(closest - target) > Math.abs(firstElement + actualSum - target)) {
    ...
    public int searchTriplet(int[] arr, int targetSum) {
        ...
        for (int i = 0; i <= arr.length - 3; i++) {
            ...
            int requiredForSum = targetSum - firstElement;
            ...
            while (left < right) {
                ...
                if (actualSum == requiredForSum) {
                    ...
                    // update closest correctly, it must be closest to the target
                    if (Math.abs(closest - targetSum) > Math.abs(firstElement + actualSum - targetSum)) {
                        closest = firstElement + actualSum;
                    } else if (Math.abs(closest - targetSum) == Math.abs(firstElement + actualSum - targetSum) && closest > firstElement + actualSum) {
                        closest = firstElement + actualSum;
                    }
                    right--;
                } else {
